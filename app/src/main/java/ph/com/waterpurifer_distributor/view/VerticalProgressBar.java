package ph.com.waterpurifer_distributor.view;
...
public class VerticalProgressBar extends View {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        height = getMeasuredHeight() ;// 高度值
    }

    @Override
    protected void onDraw(Canvas canvas) {
        paint.setColor(Color.parseColor("#44deff"));// 设置画笔颜色
        if (progress==100){
            canvas.drawRoundRect(0, height - progress / 100f * height, width, height,10,10,
                    paint);// 画矩形
        }else {
            if (progress<2){
                ...
                canvas.drawRect(0, height - progress / 100f * height, width, height - progress / 100f * height+5,
                        paint);// 画矩形
                canvas.drawRoundRect(0, height - progress / 100f * height, width, height,10,10,
                        paint);// 画矩形
            }
